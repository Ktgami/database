...
<section id="content"><div class="ic"></div>
  <div class="container">
    <div class="row">
      <div class="grid_12">
        <h3>Специальности (режим администратора)</h3>       
      </div>
...
$s="SELECT spec.* FROM spec where spec" ;
if ($value1!="Все")
$s=$s." LIKE '%$value1"."%'  ";
else
$s=$s."=spec ";
...
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="this.form.action='updspec.php?upd=1&step=1'; this.form.submit();" value="Редактировать">   
<input  type="button"  name="button" <? if (mysql_num_rows($r)==0) {?>    disabled="disabled"<? }?>  onclick="qwest=window.confirm('Вы дествительно хотите удалить запись?');  if (qwest) {this.form.action='delspec.php'; this.form.submit();}" value="Удалить">
<table WIDTH=100% border="1" cellspacing=0 cellpadding=2  >
	...
	if ($i==0)
    echo "<td><input type=radio checked=checked name=Arr[] value=".$f["idspec"]."> </td>";
	else
    echo "<td><input type=radio name=Arr[] value=".$f["idspec"]."> </td>";				
				echo "
				<td> $f[spec]</td>								
